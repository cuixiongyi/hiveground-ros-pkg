...
BCAP_HRESULT BCap::bCapSendAndRec(BCAP_PACKET *pSndPacket, BCAP_PACKET *pRecPacket)
{
 ...
 *
 *      @retval uint8_t pointer storing received packet
 *      @detail Note:   If valid pointer is returned, this function allocate memory for storing packet.
 *                                      Never forget to free() the pointer.
 *                                      And If NULL pointer is returned , then was allocation error/
 *
 */
uint8_t *BCap::receivePacket()
{
 ...
 *
 */
BCAP_HRESULT BCap::sendBinary(uint8_t *pBuff, uint32_t lSize)
{
 ...
 *
 */
BCAP_HRESULT BCap::Packet_Send(BCAP_PACKET *pPacket)
{
