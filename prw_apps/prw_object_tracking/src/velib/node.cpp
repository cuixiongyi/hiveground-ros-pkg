...
void Node::draw_editor(QPainter *painter)
{
    ...
    painter->drawRect(node_rect_.topLeft().x()-RESIZE_CENTER_MARK_SIZE, 
        node_rect_.center().y()-RESIZE_CENTER_MARK_SIZE, 
        RESIZE_CENTER_MARK_SIZE*2,
        RESIZE_CENTER_MARK_SIZE*2); 
    painter->drawRect(node_rect_.center().x()-RESIZE_CENTER_MARK_SIZE, 
        node_rect_.bottomLeft().y()-RESIZE_CENTER_MARK_SIZE,
        RESIZE_CENTER_MARK_SIZE*2,
        RESIZE_CENTER_MARK_SIZE*2); 
    painter->drawRect(node_rect_.bottomRight().x()-RESIZE_CENTER_MARK_SIZE,
        ...
        RESIZE_CENTER_MARK_SIZE*2); 
    painter->drawRect(node_rect_.center().x()-RESIZE_CENTER_MARK_SIZE,
        node_rect_.topRight().y()-RESIZE_CENTER_MARK_SIZE, 
        RESIZE_CENTER_MARK_SIZE*2,
        RESIZE_CENTER_MARK_SIZE*2); 
}

int Node::get_editor_corner(const QPointF& position, QPointF& corner, 
    QPointF& pivot_corner)
{
...
{
    QRectF left(node_rect_.topLeft().x()-RESIZE_CENTER_MARK_SIZE, 
        node_rect_.center().y()-RESIZE_CENTER_MARK_SIZE, 
        RESIZE_CENTER_MARK_SIZE*2,
        RESIZE_CENTER_MARK_SIZE*2); 
    ...
    QRectF right(node_rect_.bottomRight().x()-RESIZE_CENTER_MARK_SIZE,
        ...
        RESIZE_CENTER_MARK_SIZE*2,
        RESIZE_CENTER_MARK_SIZE*2); 
    QRectF top(node_rect_.center().x()-RESIZE_CENTER_MARK_SIZE,
        node_rect_.topRight().y()-RESIZE_CENTER_MARK_SIZE, 
        RESIZE_CENTER_MARK_SIZE*2,
        RESIZE_CENTER_MARK_SIZE*2); 
...
{
    return node_rect_.adjusted(-(RESIZE_CORNER_MARK_SIZE+2),
        -(RESIZE_CORNER_MARK_SIZE+2), 
        (RESIZE_CORNER_MARK_SIZE+2), 
        (RESIZE_CORNER_MARK_SIZE+2));
}
...
void Node::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if(edit_mode_)
    {
        ...
        {            
            ...
            {
                case 0: node_rect_.adjust(dx, 0, 0, 0); break;
                case 1: node_rect_.adjust(0, 0, 0, dy); break;
                case 2: node_rect_.adjust(0, 0, dx, 0); break;
                case 3: node_rect_.adjust(0, dy, 0, 0); break;             
            }            
            emit scene->signal_node_size_changed(this);
            handle = true;
        }
...
void Node::hoverMoveEvent(QGraphicsSceneHoverEvent * event)
{
    ...
    if(edit_mode_)
    {
        ...
        if(corner_index != -1) 
        {
            qDebug() << "corner:" << corner_index << corner;
        ...
        {         
            qDebug() << "center:" << center_index << center;
