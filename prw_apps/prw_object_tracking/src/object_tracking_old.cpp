...
ObjectTracking::ObjectTracking(ros::NodeHandle & nh, QWidget *parent, Qt::WFlags flags) :
    QMainWindow(parent, flags), nh_(nh), quit_threads_(false), pass_through_filter_(true), image_rgb_buffer_(2), image_hsv_buffer_(
        2), cloud_buffer_(2), cloud_filtered_buffer_(2)

{
  ...
  connect(color_picker_, SIGNAL(newCol(int,int)), color_luminance_picker_, SLOT(setCol(int,int)));
  ...
  connect(scene_image_, SIGNAL(mouseClicked(QPointF, QRgb)), this, SLOT(onImageClicked(QPointF, QRgb)));
  connect(ui_color_object_.pushButtonAdd, SIGNAL(clicked()), this, SLOT(onColorPickerAdd()));
  connect(ui_color_object_.colorList, SIGNAL(itemClicked(QListWidgetItem *)), this,
          SLOT(onColorListClicked(QListWidgetItem *)));
  connect(color_luminance_picker_, SIGNAL(newHsv(int, int, int)), this, SLOT(onColorPickerSelected(int, int, int)));

  connect(ui_color_object_.spinBoxMaxH, SIGNAL(valueChanged(int)), this, SLOT(onRangeUpdate()));
  connect(ui_color_object_.spinBoxMinH, SIGNAL(valueChanged(int)), this, SLOT(onRangeUpdate()));
  connect(ui_color_object_.spinBoxMaxS, SIGNAL(valueChanged(int)), this, SLOT(onRangeUpdate()));
  connect(ui_color_object_.spinBoxMinS, SIGNAL(valueChanged(int)), this, SLOT(onRangeUpdate()));
  connect(ui_color_object_.spinBoxMaxV, SIGNAL(valueChanged(int)), this, SLOT(onRangeUpdate()));
  connect(ui_color_object_.spinBoxMinV, SIGNAL(valueChanged(int)), this, SLOT(onRangeUpdate()));

  connect(color_luminance_picker_, SIGNAL(newRange(int, int)), this, SLOT(onLuminanceRangeUpdate(int, int)));
  ...
  connect(ui_color_object_.spinBoxMaxArea, SIGNAL(valueChanged(int)), this, SLOT(onSizeUpdate()));
  connect(ui_color_object_.spinBoxMinArea, SIGNAL(valueChanged(int)), this, SLOT(onSizeUpdate()));
